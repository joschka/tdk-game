    ...
    return (
      <Background name='buero-berater'>
        <div className='explainer explainer--bottom' onClick={nextStep}>
          <SpeechBubble text="Willkommen im Kanzleramt.<br /><br />Ich werde Sie während Ihrer Amtszeit beraten." noHead={true} />
        </div>
      </Background>
    );
  }
    ...
    return (
      <Background name='buero-berater'>
        <div className='explainer explainer--bottom'>
          <SpeechBubble text="<span style='display: block;text-align: center'>Die Zeit drängt!<br /><br />Sind Sie bereit das Klima zu retten?</span>" noHead={true} />
          <button className="button" style={{marginTop: '20px'}} onClick={nextStep}>Erste Amtszeit starten</button>
        </div>
      </Background>
    );
  }
