...
function ActionListHeader() {
  ...
  const handleAvailableClick = () => dispatch({ type: 'actionPartitions/change', data: 'available' });
  const handleActiveClick = () => dispatch({ type: 'actionPartitions/change', data: 'active' });
  const handleEndedClick = () => dispatch({ type: 'actionPartitions/change', data: 'ended' });
  const handleThermometerClick = () => {
    if (actionsSortBy === 'temperature') {
      dispatch({ type: 'actionsSortOrder/change', data: ['asc', 'desc'].filter(d => d !== actionsSortOrder)[0] });
    } else {
  ...
  const roundIconClassName = (actionState) => {
    return [
      ...
      actionState === actionPartitions[0] ? 'action-list-header__round-icon--active' : '',
    ].join(' ');
  }

  const thermometerClassName = [
    'action-list-header__thermometer',
    actionsSortBy === 'temperature' ? 'action-list-header__thermometer--active' : '',
  ...
  return (
    <div className='action-list-header'>
      <div className='action-list-header__round-icons'>
        <div className={roundIconClassName('available')} onClick={handleAvailableClick}>
          <IconAvailable {...roundIconProps} />
          <div className='action-list-header__count'>
            <span>{ availableCount }</span>
          </div>
        </div>
        { (activeCount > 0 || endedCount > 0) && <div className={roundIconClassName('active')} onClick={handleActiveClick}>
          <IconActive {...roundIconProps} />
          <div className='action-list-header__count'>
            <span>{ activeCount }</span>
          </div>
        </div> }
        { endedCount > 0 && <div className={roundIconClassName('ended')} onClick={handleEndedClick}>
